...
function extractMeta(obj) {
  ...
  for (const [key, val] of Object.entries(obj)) {
    ...
    if (["status"].includes(k)) {
      meta.status = normalizeStatus(val);
      consumed.add(key);
    } else if (["priority"].includes(k)) {
      meta.priority = normalizePriority(val);
      consumed.add(key);
    } else if (["owner", "assignee", "assigned_to"].includes(k)) {
      meta.assignee = String(val);
      consumed.add(key);
    } else if (["deadline", "date", "due_date", "duedate", "due"].includes(k)) {
      meta.dueDate = normalizeDate(val);
      consumed.add(key);
    } else if (["tags", "labels"].includes(k)) {
      meta.tags = normalizeTags(val);
      consumed.add(key);
    } else if (["depends_on", "dependencies", "blocked_by"].includes(k)) {
      meta.dependencies = normalizeTags(val);
      consumed.add(key);
    } else if (
      ["notes", "description", "details", "summary", "desc"].includes(k)
    ) {
      meta.description = String(val);
      consumed.add(key);
    }
  }
...
function extractTitle(obj) {
  for (const key of [
    ...
    if (obj[key] && typeof obj[key] === "string")
      return { title: obj[key], titleKey: key };
  }
...
export function parseJsonToTasks(jsonInput) {
  ...
  if (data === null || typeof data !== "object") {
    return {
      ...
      details: "Input must be a JSON object or array",
    };
  }
  ...
  function buildTask(obj, sourcePath, parentTaskId = null) {
    ...
    for (const key of Object.keys(obj)) {
      if (
        !consumed.has(key) &&
        !META_KEYS.has(key.toLowerCase()) &&
        !SUBTASK_KEYS.has(key.toLowerCase())
      ) {
  ...
  function processArray(arr, sourcePath, explicitPhaseName = null) {
    ...
    arr.forEach((item, i) => {
      ...
      let phaseName = explicitPhaseName;

      // Infer phase from task metadata if no explicit phase
      if (!phaseName && typeof item === "object" && item !== null) {
        phaseName =
          item.phase ||
          item.category ||
          item.section ||
          ...
          item.module ||
          item.stage ||
          item.status || // optional fallback
          "Imported Tasks";
      }

      if (!groupedTasks.has(phaseName)) {
        groupedTasks.set(phaseName, []);
      }

      groupedTasks.get(phaseName).push(task);
    });

    // Create phases
    groupedTasks.forEach((tasks, phaseName) => {
      const existingPhase = allPhases.find(
        (p) => p.name.toLowerCase() === String(phaseName).toLowerCase(),
      );

      if (existingPhase) {
        existingPhase.tasks.push(...tasks);
      } else {
        allPhases.push({
  ...
  function processObject(obj, sourcePath) {
    ...
    for (const [key, val] of Object.entries(obj)) {
      ...
      if (NAME_KEYS.has(k) || k === "projectname" || k === "project_name")
        continue;
        ...
        val.forEach((groupItem, gi) => {
          if (typeof groupItem === "object" && groupItem !== null) {
            ...
            for (const [gk, gv] of Object.entries(groupItem)) {
              if (
                Array.isArray(gv) &&
                (TASK_KEYS.has(gk.toLowerCase()) ||
                  SUBTASK_KEYS.has(gk.toLowerCase()))
              ) {
                processArray(
                  gv,
                  `${sourcePath}.${key}[${gi}].${gk}`,
                  String(groupName),
                );
                foundGroups = true;
              }
      ...
      if (
        ...
        for (const [innerKey, innerVal] of Object.entries(val)) {
          if (
            ...
            TASK_KEYS.has(innerKey.toLowerCase())
          ) {
        ...
        if (!hasTaskArrays && !foundGroups) {
          ...
          if (innerTask) {
            const defaultPhase = allPhases.find(
              (p) => p.name === "Imported Tasks",
            );
  ...
  const { count: tasksDetected, subCount: subtasksDetected } =
    countTasks(allTasksFlat);
  ...
  if (tasksDetected === 0) {
    return {
      ...
      details:
        'Could not find any task-like structures in the provided JSON. Ensure your JSON contains keys like "task", "title", "name", "step", "item", etc.',
    };
  }
