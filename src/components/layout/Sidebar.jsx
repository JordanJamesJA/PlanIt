          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '8px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
            {projects.map(p => (
              <button
                key={p.id}
                title={p.name}
                onClick={() => { setActiveProject(p.id); setCollapsed(false); }}
                style={{
                  width: 32, height: 32,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  background: activeProject?.id === p.id ? p.color + '30' : 'transparent',
            ...
            <button
              title="New project"
              onClick={() => { setCreateModal(true); setCollapsed(false); }}
              style={{
